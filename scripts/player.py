...
class Player:
    ...
    def __init__(self, game, pos, size):
        ...
        self.length = 0     # 長さ
        self.prev_pos = [0, 0]  # 1フレーム前の位置

        # リリース
        self.release_velocity = [0, 0]
        self.is_release_right = False
    ...
    def move_on_release(self):
        # 左移動
        if self.game.INPUTS["a"]:
            self.velocity[0] -= 0.5
            if self.is_release_right:
                self.velocity[0] = max(self.velocity[0], -self.speed)
            else:
                if abs(self.release_velocity[0]) < self.speed:
                    self.velocity[0] = max(self.velocity[0], -self.speed)
                else:
                    self.velocity[0] = max(self.velocity[0], self.release_velocity[0])
        # 右移動
        if self.game.INPUTS["d"]:
            ...
            if self.is_release_right:
                if abs(self.release_velocity[0]) < self.speed:
                    ...
                    self.velocity[0] = min(self.velocity[0], self.release_velocity[0])
            else:
                self.velocity[0] = min(self.velocity[0], self.speed)
    ...
    def swinging(self):
        ...
        for tile in self.game.tile_map.tile_list:
            if tile.rect.colliderect(self.rect):
                frame_movement = self.pos[0] - self.prev_pos[0]
                # 左移動
                if frame_movement < 0:
                    self.rect.left = tile.rect.right
                # 右移動
                ...
                if abs(self.angle_velocity) < 0.03:
                    if self.angle_velocity > 0:
                        self.angle_velocity = 0.03
                    else:
                        self.angle_velocity = -0.03
        ...
        for tile in self.game.tile_map.tile_list:
            if tile.rect.colliderect(self.rect):
                frame_movement = self.pos[1] - self.prev_pos[1]
                # 上移動
                if frame_movement < 0:
                    self.rect.top = tile.rect.bottom
                # 下移動
        ...
        if not self.game.INPUTS["left_click"]:
            ...
            self.game.hook.is_fixed = False
            self.calculate_release_init_parameters()
    ...
    def calculate_release_init_parameters(self):
        self.release_velocity[0] = self.angle_velocity * self.length * math.cos(self.angle)
        self.velocity[0] = self.angle_velocity * self.length * math.cos(self.angle)

        self.release_velocity[1] = -self.angle_velocity * self.length * math.sin(self.angle)
        self.velocity[1] = -self.angle_velocity * self.length * math.sin(self.angle)

        if self.velocity[0] > 0:
            self.is_release_right = True
        else:
            self.is_release_right = False
